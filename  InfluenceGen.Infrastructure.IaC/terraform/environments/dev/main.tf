...
terraform {
  backend "s3" {
    bucket = "influencegen-terraform-state-dev" # Replace with actual bucket name
...
# Define environment-specific variables (loaded from dev.tfvars)
...
locals {
  security_group_definitions = {
    odoo_web_sg = {
      ...
      egress = [] # Default egress handled by module
    }
    odoo_internal_sg = {
      description = "Internal Odoo app security group"
      ingress = [
         // Allow Odoo instances to talk to each other if needed
         ...
         { protocol = "tcp", from_port = 8000, to_port = 8000, source_security_group_id = module.security_groups.security_group_ids["ai_server_sg"], cidr_blocks = null, description = "Allow Odoo to AI Server" }, // Assuming AI Server runs on 8000
    ...
    n8n_sg = {
      ...
      ingress = [
        { protocol = "tcp", from_port = 5678, to_port = 5678, cidr_blocks = ["0.0.0.0/0"], source_security_group_id = null, description = "Allow N8N web interface (Adjust CIDR for production)" },
...
module "rds_postgres" {
  ...
  subnet_ids             = module.vpc.private_subnet_ids # Deploy DB in private subnets
  username_secret_arn    = var.db_username_secret_arn
  password_secret_arn    = var.db_password_secret_arn
  multi_az               = false # Dev typically doesn't need Multi-AZ
...
# Provision Odoo Application Servers
module "odoo_app" {
  source = "../../modules/odoo_app"

  instance_count = var.odoo_instance_count
  ami_id         = var.odoo_ami_id
  ...
  subnet_ids     = module.vpc.private_subnet_ids # Deploy app servers in private subnets
  ...
  db_user_secret_arn = var.odoo_db_user_secret_arn
  db_password_secret_arn = var.odoo_db_password_secret_arn
  odoo_admin_password_secret_arn = var.odoo_admin_password_secret_arn # Not directly used by module, passed to Ansible
...
module "n8n_app" {
  ...
  subnet_id      = module.vpc.private_subnet_ids[0] # Deploy N8N in a private subnet
  ...
  n8n_db_name     = var.n8n_db_type == "postgres" ? var.db_name : null # Use the same DB as Odoo for N8N
  n8n_db_user_secret_arn = var.n8n_db_type == "postgres" ? var.n8n_db_user_secret_arn : null
  n8n_db_password_secret_arn = var.n8n_db_type == "postgres" ? var.n8n_db_password_secret_arn : null
